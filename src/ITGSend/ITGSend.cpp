...
void *flowParser(void *param)
{
	...
	while (token != NULL) {
		...
		if(strcmp(token,"-NO")==0 && cmdMode == false){	
	...
	while (argc > 0) {
		...
		if (argv[h][0] == '-') {
			...
			case 'N':
				if(argv[h][2] == 'O'){
					char* stok = strtok(argv[h+1], " ");
					npp[id].neatOpt[0] = "NEAT-client";
					int i = 1;
					while(stok != NULL && i < max_optc){
						npp[id].neatOpt[i] = stok;
						stok = strtok(NULL, " ");
						i++;
					}
					h += 2;
	...
	if(flows[id].l4Proto == L4_PROTO_NEAT) {

		if(!neatoptCheck.empty())
		{
			cout << endl;
			for(vector<char>::iterator it = neatoptCheck.begin() ; it != neatoptCheck.end(); ++it) {
				cout << " ** ERROR ** - option -" << *it << " is not compatible with NEAT" << endl;
			}
		...
		getInfo(flows[id].DestHost, tmpPort_DstPort, HelpDstAddress);

		npp[id].addr = HelpDstAddress;
...
void *signalManager(void *id)
{
	...
	FD_ZERO(&rset);
		...
		FD_SET(fd, &rset);
		int x;
		if ((x = select(maxfd + 1, &rset, NULL, NULL, NULL)) == -1) {
			if (errno == EINTR)
				continue;
...
if(flows[id].l4Proto == L4_PROTO_NEAT){
		npp[id].flowid = id;
		if(CREATE_THREAD(&npp[id], neat_client_start, NULL, h_neatClients[id-1], false) < 0)
			ReportErrorAndExit("flowSender", "Cannot create thread for neat_client_start",programName, id);
		pthread_mutex_lock(&get_nt_mp(id));
		while(!get_nt_ret(id)) pthread_cond_wait(&get_nt_cv(id), &get_nt_mp(id));
		pthread_mutex_unlock(&get_nt_mp(id));
	...
	do {
		...
		do {
			...
			if ((l4Proto == L4_PROTO_ICMP) && (meter == METER_OWDM)) {
				...
				if(l4Proto!=L4_PROTO_NEAT){
					sockchk = send(sock, (char *) payload, size, 0);
				}
				else{
					neat_send(payload, size, id);
				}
				...
				if ((logging) && (meter == METER_OWDM)) {
					...
					if(flows[id].l4Proto == L4_PROTO_NEAT){
						writeInBufferStandard(&infos[count], htonl(id), htonl(seqNum), HelpSrcAddress,
								HelpDstAddress, get_flow_protocol(get_flow(id)), ntohs(0), ntohs(8080), time, time,
								Ticker.lastTime.tv_usec, Ticker.lastTime.tv_usec, size); 
					}
					else{
						writeInBufferStandard(&infos[count], htonl(id), htonl(seqNum), HelpSrcAddress,
							HelpDstAddress, invFindL4Proto(flows[id].l4Proto), tmpPort_SrcPort, tmpPort_DstPort, time, time,
							Ticker.lastTime.tv_usec, Ticker.lastTime.tv_usec, size); 
					}
							...
							if (logremoto == 0) {
								...
								writeInBufferStandard(&infos[count], *(uint32_t *) payload,
										*(uint32_t *) ptrSeqNum, HelpSrcAddress,
										HelpDstAddress, invFindL4Proto(flows[id].l4Proto), tmpPort_SrcPort, tmpPort_DstPort,
				...
				if (FD_ISSET(sock, &read_set)) {
					...
					if (logging && size_r > 0) {
						if (logremoto == 0) {
							...
							writeInBufferStandard(&infos[count], *(uint32_t *) payload,
									*(uint32_t *) ptrSeqNum, HelpSrcAddress, HelpDstAddress, invFindL4Proto(flows[id].l4Proto),
	...
	if(l4Proto == L4_PROTO_NEAT)
	{
		neat_stop_event_loop(id);
		pthread_join(h_neatClients[id-1], NULL);
	}
