...
void terminate(int sign)
{
	if (flagTerm == NO_TERMINATE) {
		...
		if(nt_server != NULL){
			...
			PRINTD(1,"signalManager: Stopping NEAT event loop \n");
			neat_stop_event_loop();

			PRINTD(1,"signalManager: Waiting for NEAT server to join the main thread...\n");
			int jstatus = pthread_join(nt_server, NULL);

			PRINTD(1,"signalManager: NEAT joined with status: %s \n", strerror(jstatus));
...
void *signalManager(void *param )
{
	...
	if(nt_server != NULL){
			...
			PRINTD(1,"signalManager: Printing NEAT logs \n");
			print_logs();
...
#endif
{
	if (type == TSP_SEND_FLOW) {
		...
		if (flowPosition < MAX_NUM_THREAD) {
			...
			case L4_PROTO_NEAT:
				if(nt_server == NULL)
					if(CREATE_THREAD(&npp,
						neat_server_start, NULL, nt_server, false) < 0)
						reportErrorAndExit("typeParser",
							"CREATE_THREAD - type = TSP_SEND_FLOW(3",
							"Cannot create thread for neat_server_start");
					
				if (CREATE_THREAD(&paraThread[flowPosition],
					neatSock, NULL, hThr[flowPosition], true) < 0)
					reportErrorAndExit("typeParser",
					    "CREATE_THREAD - type = TSP_SEND_FLOW(3",
					    "Cannot create thread for neatSock");
				PRINTD(1,"typeParser: Return value CREATE_THREAD neatSock hThr[flowPosition] : %lu \n",
				    (unsigned long int) hThr[flowPosition]);
				break;
			default:
				break;
...
void parserRecv(int argc, char *argv[])
{
	...
	while (argc > 0) {
		...
		case 'N':
			if(argv[0][2] == 'O'){
				char* stok = strtok(argv[1], " ");
				npp.neatOpt[0] = "NEAT-server";
				int i = 1;
